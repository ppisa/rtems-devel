...
void
rtems_lwip_semaphore_obtain (void)
{
 ...
 */
void
rtems_lwip_semaphore_release (void)
{
        rtems_recursive_mutex_unlock(&rtems_lwip_mutex);
}

static inline int
rtems_lwip_iop_to_lwipfd (rtems_libio_t *iop)
{
 ...
 */
int
rtems_lwip_sysfd_to_lwipfd (int fd)
{
 ...
 */
static int
rtems_lwip_make_sysfd_from_lwipfd (int lfwipfd)
{
  rtems_libio_t *iop;
  int fd;
...
bind (int s, const struct sockaddr *name, socklen_t namelen)
{
  ...
  if ((family == rtems_lwip_sysdefs_AF_INET) &&
    ...
    so_in.sin_addr.s_addr = rtems_lwip_sysdefs_sockaddr_in_get_sin_addr(name);
    ...
    so_in6.sin6_flowinfo = rtems_lwip_sysdefs_sockaddr_in6_get_sin6_flowinfo(name);
    so_in6.sin6_scope_id = rtems_lwip_sysdefs_sockaddr_in6_get_sin6_scope_id(name);
...
accept (int s, struct sockaddr *name, socklen_t *namelen)
{
  ...
  if (lwipname->sa_family == AF_INET) {
    ...
    rtems_lwip_sysdefs_sockaddr_set_len(name, rtems_lwip_sysdefs_sockaddr_in_size);
    ...
    rtems_lwip_sysdefs_sockaddr_set_len(name, rtems_lwip_sysdefs_sockaddr_in6_size);
    rtems_lwip_sysdefs_sockaddr_set_family(name, rtems_lwip_sysdefs_AF_INET6);
    ...
    memcpy(rtems_lwip_sysdefs_sockaddr_in6_get_sin6_addr_ptr(name),
           so_in6.sin6_addr,
           sizeof(so_in6.sin6_addr));
    rtems_lwip_sysdefs_sockaddr_in6_set_sin6_flowinfo(name, so_in6.sin6_flowinfo);
    rtems_lwip_sysdefs_sockaddr_in6_set_sin6_scope_id(name, so_in6.sin6_scope_id);
 #endif
  } else {
    lwip_close(lwipfd);
    errno = EINVAL;
 ...
 */
static int
rtems_lwip_close (rtems_libio_t *iop)
{
  int lwipfd;
  int ret;
...
static ssize_t
rtems_lwip_read (rtems_libio_t *iop, void *buffer, size_t count)
{
  int lwipfd;
  int ret;
...
static ssize_t
rtems_lwip_write (rtems_libio_t *iop, const void *buffer, size_t count)
{
  int lwipfd;
  int ret;
...
int
so_ioctl (rtems_libio_t *iop, int lwipfd, uint32_t   command, void *buffer)
{
#if 0
	switch (command) {
	case FIONBIO:
...
static int
rtems_lwip_ioctl (rtems_libio_t *iop, ioctl_command_t command, void *buffer)
{
#if 0
	int lwipfd;
	int error;
...
static int
rtems_lwip_fcntl (rtems_libio_t *iop, int cmd)
{
#if 0
	int lwipfd;
...
static int
rtems_lwip_fstat (const rtems_filesystem_location_info_t *loc, struct stat *sp)
{
	sp->st_mode = S_IFSOCK;
	return 0;
}
